...
export default function ClubDetailsPage() {
  ...
  const [addRole, setAddRole] = useState<
    "LEAD" | "CORE" | "COORDINATOR" | "SENIOR_MEMBER" | "MEMBER" | "VOLUNTEER"
  >("MEMBER");
  ...
  const canTransferRole =
    myMembership && !["MEMBER", "VOLUNTEER"].includes(myMembership.role);
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <Card className="bg-black/40 backdrop-blur-xl border border-white/5 shadow-2xl rounded-3xl overflow-hidden">
          ...
          <Card className="bg-black/40 backdrop-blur-xl border border-white/5 shadow-2xl rounded-3xl overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-white/5 bg-white/5">
              <CardTitle className="flex items-center gap-2 text-xl font-bold">
                <Users className="h-5 w-5 text-pastel-peach" /> Club Roster
              </CardTitle>
              <span className="text-sm text-neutral-400 font-medium px-3 py-1 bg-white/10 rounded-full">
            ...
            <CardContent className="pt-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {club.members.map((member, idx) => (
                  ...
                  >
                    <div className="h-12 w-12 rounded-full bg-white/10 overflow-hidden shrink-0 border border-white/20 shadow-inner">
                      {member.user.image ? (
                        <Image
          ...
          <Card className="bg-black/40 backdrop-blur-xl border border-white/5 shadow-2xl rounded-3xl overflow-hidden">
            <CardHeader className="border-b border-white/5 pb-4 bg-white/5">
              <CardTitle className="flex items-center gap-2 text-xl font-bold">
                <CalendarIcon className="h-5 w-5 text-pastel-lavender" />{" "}
                Upcoming Events
              </CardTitle>
            </CardHeader>
            <CardContent className="pt-6">
              <div className="space-y-4">
                {club.events.map((event) => (
                  <div
                    key={event.id}
                    className="flex gap-4 p-4 rounded-2xl border border-white/5 bg-white/5 hover:bg-white/10 transition-colors"
                  >
                    <div className="flex flex-col items-center justify-center h-16 w-16 rounded-xl bg-pastel-lavender/10 border border-pastel-lavender/20 shrink-0 text-center shadow-inner">
                      <span className="text-xs font-bold text-pastel-lavender uppercase leading-none mb-1">
                        {new Date(event.date).toLocaleString("default", {
                          month: "short",
                        })}
                      </span>
                      <span className="text-2xl font-black text-pastel-lavender leading-none">
        ...
        <div className="space-y-6">
          {(isLead || isCore || isAdmin) && (
            <Card className="bg-amber-950/20 border-amber-900/50 backdrop-blur-xl">
              <CardHeader className="pb-3 border-b border-amber-900/30">
                <CardTitle className="text-amber-500 flex items-center gap-2">
                  <ShieldCheck className="h-5 w-5" /> Manage Roster
                </CardTitle>
                <CardDescription className="text-amber-500/70">
                  Assign roles to students. This will grant them a verified
                  profile badge.
                </CardDescription>
              </CardHeader>
            ...
            <Card className="bg-red-950/20 border-red-900/50">
              <CardHeader className="pb-3 border-b border-red-900/30">
                <CardTitle className="text-red-400 flex items-center gap-2">
                  <CalendarIcon className="h-5 w-5" /> Host Event
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-4">
                <form onSubmit={handleCreateEvent} className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-red-400/80 uppercase">
                      Event Title
                    </label>
                    <input
                      required
            ...
            <Card className="bg-purple-950/20 border-purple-900/50">
              <CardHeader className="pb-3 border-b border-purple-900/30">
                <CardTitle className="text-purple-400 flex items-center gap-2">
                  <ShieldAlert className="h-5 w-5" /> Transfer Role
                </CardTitle>
                <CardDescription className="text-purple-400/70">
                  Pass down your current position (
                  {myMembership?.role.replace("_", " ")}) to another student.
                  You will become a Senior Member.
                </CardDescription>
              </CardHeader>
              <CardContent className="pt-4">
                <form onSubmit={handleTransferRole} className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-purple-400/80 uppercase">
                      Student Email
                    </label>
                    <input
                      required
