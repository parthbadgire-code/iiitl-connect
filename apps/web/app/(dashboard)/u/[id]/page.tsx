...
export default function PublicProfilePage() {
  ...
  const fetchProfile = async () => {
    ...
    setError(false);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/profile/public/${id}`, { credentials: "include" });
      if (res.ok) {
  ...
  if (error || !profile) {
    return <div className="flex justify-center items-center h-64 text-zinc-500">Profile not found.</div>;
  }
